document.addEventListener('DOMContentLoaded', function () {
    ...
    // Charger les options des Armes depuis sciel.html
    fetch('../tabs/sciel.html')
        .then(response => {
            if (!response.ok) {
                throw new Error(`Erreur HTTP : ${response.status}`);
            }
    ...
    document.getElementById('build-form').addEventListener('submit', function (e) {
        ...
        const build = {
            ...
            weapons,
            category: 'Sciel' // Catégorie spécifique pour Sciel
        };
        ...
        alert('Build enregistré avec succès dans la catégorie Sciel !');
    });
});