document.addEventListener('DOMContentLoaded', function () {
    ...
    // Charger les options des Armes depuis maelle.html
    fetch('../tabs/maelle.html')
        .then(response => {
            if (!response.ok) {
                throw new Error(`Erreur HTTP : ${response.status}`);
            }
    ...
    document.getElementById('build-form').addEventListener('submit', function (e) {
        ...
        const build = {
            ...
            weapons,
            category: 'Maelle' // Ajout de la catégorie
        };
        ...
        alert('Build enregistré avec succès dans la catégorie Maelle !');
    });
});