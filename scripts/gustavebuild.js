document.addEventListener('DOMContentLoaded', function () {
    ...
    // Charger les options des Armes depuis gustave.html
    fetch('../tabs/gustave.html')
        .then(response => {
            if (!response.ok) {
                throw new Error(`Erreur HTTP : ${response.status}`);
            }
    ...
    document.getElementById('build-form').addEventListener('submit', function (e) {
        ...
        const build = {
            ...
            weapons,
            category: 'Gustave' // Catégorie spécifique pour Gustave
        };
        ...
        alert('Build enregistré avec succès dans la catégorie Gustave !');
    });
});